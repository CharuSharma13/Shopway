...
import logo from "../Utilities/shopway_Logo.jpg";
    ...
    <nav className="flex flex-row justify-between items-center h-16 mx-auto max-w-6xl ">
      ...
      <div className="flex flex-row  items-center font-medium mr-5 space-x-6">
        <div className="text-2xl cursor-pointer pt-2" onClick={changetheme}>
          <button className="transition duration-500 ease-in-out">{theme ? <FaMoon className="w-6 h-6 text-slate-100" /> : <BsSun className="w-6 h-6" />}</button>
        </div>
        ...
        <NavLink to="/cart" className="flex items-center">
          <div className="relative text-2xl">
            ...
            {cart.length > 0 && (
              <span
                className="absolute p-2 -top-1 -right-2 text-xs w-3 h-3 bg-green-600 
